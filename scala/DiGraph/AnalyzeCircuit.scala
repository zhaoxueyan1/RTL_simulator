...
class AnalyzeCircuit extends Transform {
    ...
    def walkStatement(prefix: String)(s: Statement): Statement = {
        s match {
            ...
            case instance: WDefInstance =>
                ...
                walkModule(prefix+s"${instance.name}")(graph.moduleMap(instance.module))
    ...
    def parseExpr(prefix: String)(e: Expression): Node = {
        e match {
            ...
            case w: WSubField =>
                if(!graph.NodeMP.contains(s"${emitExpr(w.expr,prefix)}${w.name}")){
                    println(w)
                }
                graph.NodeMP(s"${emitExpr(w.expr,prefix)}${w.name}")
            case w: WSubAccess =>
